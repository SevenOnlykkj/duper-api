...
if (!url) return res.json({ status : false, creator : `Redzin`, message : "Cade o parametro url?"}) 
        if(key.includes(apikey)){
       igstory(url)
	.then(data => {
		var result = data;
...
await listkeys(apikey, req);
text = req.query.text
if (!text) return res.json({ status : false, creator : `Redzin`, message : "Cade o parametro text?"})
        if(key.includes(apikey)){
...
await listkeys(apikey, req);
query = req.query.query
if (!query) return res.json({ status : false, creator : `Redzin`, message : "Cade o parametro query?"})
wikimedia(query)
.then(hasil => {
var resultado = hasil;
res.json({
...
await listkeys(apikey, req);
url = req.query.url
if (!url) return res.json({ status : false, criador : `Redzin`, mensagem : "Coloque o parametro: q"})
...
res.json({
status: true,
mensagem: 'HIARA APIS - MELHOR SITE DE APIS',
criador: `Redzin`,
resultado: `${auu.str}`,
})
} catch (error) {
...
if(!texto)return res.json({status:false,motivo:'Cadê o parâmetro img?'})
    hasil = `https://api.bronxyshost.com.br/api-bronxys/attp_edit?texto=${encodeURIComponent(q)}&fonte=${Fontes}&apikey=Sandro_api_bot`
	  data = await fetch(hasil).then(v => v.buffer())   
         await fs.writeFileSync(bla+'/tmp/gostosinha.jpg', data)
        res.sendFile(bla+'/tmp/gostosinha.jpg') 
}) 
...
res.json({
status: true,
site: `https://redzinrestapi.onrender.com`,
...
status = {
    ...
    request: totalreq,
        ...
        info:{       
            owner: 'Redzin',
            whatsapp: 'https://wa.me/5512988255401'
        }
    }
res.json(status)
}});
